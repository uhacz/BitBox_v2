#pragma once
inline float length_sqr( const vec4_t& v )
{
    return v.x * v.x + v.y * v.y + v.z * v.z + v.w * v.w;
}

inline float length( const vec4_t& v )
{
...
inline float normalized( vec4_t* v )
{
