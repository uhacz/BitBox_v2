...
static void StartUp( ANIMatchDatabase* )
{}
...
static inline T ToEntry( const U& value, f32 phase, u32 frame, u32 clip, u32 joint )
{
    T result;
    result.value = value;
    result.phase = phase;
...
void DrawGUI( ANIMatchDatabaseDebugInfo* dbg, const ANIMatchDatabase* db )
{
    ...
    if( db->skel )
    {
        const char* names = TYPE_OFFSET_GET_POINTER( const char, db->skel->offsetJointNamesStrings );
        if( names )
        {
            if( ImGui::TreeNodeEx( "Joints", ImGuiTreeNodeFlags_Framed ) )
            {
                const char* current = names;
                for( u32 i = 0; i < db->skel->numJoints; ++i )
                {
    ...
    if( db->clip_metadata.size )
    {
        if( ImGui::TreeNodeEx( "Clips", ImGuiTreeNodeFlags_Framed ) )
        {
            for( u32 i = 0; i < db->clip_metadata.size; ++i )
            {
                const ANIMatchDatabase::ClipMetadata& clip = db->clip_metadata[i];
                if( ImGui::Selectable( clip.filename.AbsolutePath(), i == (u32)dbg->selected_clip ) )
                {
                    dbg->selected_clip = i;
                }
            }
    ...
    if( (u32)dbg->selected_clip < db->clips.size )
    {
        ImGui::Separator();
        ImGui::BeginChild( "Selected Clip", ImVec2(), true );
        {
            const ANIMClip* clip = db->clip_metadata[dbg->selected_clip].file->data<ANIMClip>();
            ImGui::SliderInt( "Frame", &dbg->selected_frame, 0, clip->numFrames );
        }
        ImGui::Checkbox( "Isolate frame", &dbg->isolate_frame );


        const u32 nb_joints = (u32)sizeof_array( setup::joint_names );
        const char* current = "all";
        if( dbg->selected_joint != UINT32_MAX )
        {
            for( u32 i = 0; i < nb_joints; ++i )
            {
                if( setup::joint_indices[i] == dbg->selected_joint )
                {
        ...
        {
            if( ImGui::Selectable( "none", dbg->selected_joint == UINT32_MAX ) )
            {
                dbg->selected_joint = UINT32_MAX;
            }

            for( u32 ijoint = 0; ijoint < nb_joints; ++ijoint )
            {
                const u32 index = setup::joint_indices[ijoint];
                if( ImGui::Selectable( setup::joint_names[ijoint], (u32)dbg->selected_joint == index ) )
                {
                    dbg->selected_joint = index;
                }
            }
            ImGui::EndCombo();
        }
...
{
    const u32 clip_index = dbg.selected_clip;
    const ANIMClip* clip = db->clips[clip_index];
    const ANIMatchEntry3* pos_begin = FindBegin( db->pos, dbg.selected_clip );
    ...
    const u32 joint_index = dbg.selected_joint;
    const u32 clip_frame = dbg.selected_clip;
    for( u32 i = 0; i < nb_entries; ++i )
    {
        ...
        if( dbg.isolate_frame && it_pos->frame != clip_frame )
            continue;
